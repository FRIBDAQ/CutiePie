...
//             if (strcmp(gottenhostname, item.m_host.c_str())==0) {
...
*/
void MirrorThread(MirrorClientTest* pClient, unsigned interval) {
...
startMirroring(const char* host, int mirror, int rest, size_t size)
{
    ...
    if (result) {
        ...
        MirrorClientTest* pClient(0);
        try {
            std::string shost(host);
            pClient = new MirrorClientTest(shost, mirror, result);

            // Now start an update thread:
            pThread = new std::thread(MirrorThread, pClient, UPDATE_INTERVAL);
            sleep(1);               // Wait for the mirror to populate first.
        } 
        catch (...) {
            delete[]result;
            delete pClient;
