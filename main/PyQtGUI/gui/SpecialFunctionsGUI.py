...
class SpecialFunctions(QWidget):
    ...
    def create_fitBox(self):
        ...
        self.fit_p8_label = QLabel("p8")  # for alpha22
        ...
        # --- wmode dropdown (mirrors hidden fit_p12) ---
        self.wmode_combo = QComboBox(self)
        self.wmode_combo.addItem("unweighted", 0)
        self.wmode_combo.addItem("Poisson(data)", 1)
        self.wmode_combo.addItem("Poisson(model, IRLS)", 2)
        self.wmode_combo.setToolTip("0=unweighted, 1=Poisson(data), 2=Poisson(model, IRLS)")
        self.wmode_combo.setVisible(False)  # default hidden

        def _sync_wmode_edit():
            val = self.wmode_combo.currentData()
            self.fit_p12.setText(str(val) if val is not None else "")

        self.wmode_combo.currentIndexChanged.connect(_sync_wmode_edit)
        self.wmode_combo.setCurrentIndex(1)   # default display "1 — Poisson(data)"
        self.fit_p12.setText("1")             # keep the hidden p12 in sync
        ...
        deflayout.addWidget(self.fit_p8_label, 4, 0)
        ...
        # place the combo in the same cell; visibility decides which shows
        deflayout.addWidget(self.wmode_combo, 6, 1)
    ...
    def _on_model_changed(self, name: str):
        ...
        maps = {
            ...
            "AlphaEMGMultiSigma": [
                ...
                "tau1 <=",       # p10
                "tau2 <=",       # p11
                "wmode:",        # p12 (we'll show label + combo, hide the line-edit)
                "offset correction <=",  # p13
                "slope correction <=",   # p14
                "local offset <=",       # p15
                "", "", "", "", ""       # p16..p19 hidden
            ],
        }
        ...
        if name == "AlphaEMG12":
            ...
            # ensure wmode has a sane default
        ...
        # Show the combo only for AlphaEMGMultiSigma.
        is_multi = (name == "AlphaEMGMultiSigma")
        self.wmode_combo.setVisible(is_multi)
        # If combo is visible, hide the backing line-edit; otherwise show/hide via map
        self.fit_p12.setVisible((not is_multi) and (len(names) > 12 and bool(names[12])))
