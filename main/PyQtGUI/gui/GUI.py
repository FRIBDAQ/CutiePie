...
def cutiepie_version():
    """CutiePie version for the window title. Layered so it is never blank:

    1. `_version.py`, generated by `configure` from `_version.py.in` with
       `@PACKAGE_VERSION@` substituted — the only copy that ships in an
       installed tree, so this is the live path.
    2. `configure.ac`'s AC_INIT line, read directly — for a raw source checkout
       that has not been `configure`d yet (configure.ac sits two levels up).
    3. A hardcoded default, if neither is available.

    configure.ac stays the single source of truth (1 and 3 both derive from it)."""
    try:
        from _version import __version__
        # guard against an unsubstituted template on the path (@PACKAGE_VERSION@)
        if __version__ and "@" not in __version__:
            return __version__
    except Exception:
        pass
    try:
