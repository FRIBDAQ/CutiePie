...
filepath as arguments and imports only pandas/numpy. The MainWindow keeps the
Qt shell — reading the filename widget, the WebWindow lifecycle, notebook
process control (``notebook_process.py``) — and calls in here. Extracted from
``GUI.py:createDf`` so the reshape/flatten logic is unit-testable without PyQt5
(mirrors ``geometry_io`` / ``display_slot`` / ``notebook_process``).
"""
...
# dict returned by SpectrumStore.as_dict().
_COLUMNS = ['name', 'dim', 'binx', 'minx', 'maxx',
            'biny', 'miny', 'maxy', 'data', 'parameters', 'type']


def build_spectrum_dataframe(spectrum_dict):
...
def export_spectrum_csv(spectrum_dict, filepath):
    """Build the spectrum DataFrame and write it as a gzip-compressed CSV."""
    df = build_spectrum_dataframe(spectrum_dict)
